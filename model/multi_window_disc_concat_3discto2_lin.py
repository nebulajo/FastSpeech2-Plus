...
class SingleWindowDisc(nn.Module):
    def __init__(
        self, time_length, freq_length=80, kernel=(3, 3), c_in=1, hidden_size=128
    ):
        super().__init__()
        ...
        ds_size = (
            time_length // 2**3,
            ((freq_length + 7) // 2**3) + ((freq_length + 128 + 7) // 2**3),
        )
...
class MultiWindowDiscriminator(nn.Module):
    def __init__(
        self, time_lengths, freq_length=336, kernel=(3, 3), c_in=1, hidden_size=128
    ):
    ...
    def forward(self, x, x_len, emo_cond_embed, spk_cond_embed, start_frames_wins=None):
        ...
        for i, start_frames in zip(range(len(self.discriminators)), start_frames_wins):
            ...
            if x_clip is None:
                continue
            e_cond, s_cond = self.discriminators[i](
                x_clip, emo_cond_embed, spk_cond_embed
            )
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, emo_cond_embed, spk_cond_embed, start_frames_wins=None):
        ...
        e_y_cond, s_y_cond = self.discriminator(
            x,
            x_len,
            emo_cond_embed,
            spk_cond_embed,
            start_frames_wins=start_frames_wins,
        )
        return (e_y_cond, s_y_cond)
...
if __name__ == "__main__":
    ...
    disc_params = list(mel_disc.parameters())
